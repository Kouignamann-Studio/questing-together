...
export function ChoiceIntentPortraits({
  ...
  const visiblePlayers = players.filter((player): player is ChoiceIntentPortraitPlayer & { roleId: RoleId } => Boolean(player.roleId));
      ...
      {visiblePlayers.map((player, index) => (
        <View
          ...
          style={[
            ...
            index > 0 && (size === 'compact' ? styles.portraitStackCompact : styles.portraitStackDefault),
          ...
          <Image source={portraitByRole(player.roleId)} style={styles.portraitImage} resizeMode="contain" />
        </View>
      ))}
    </View>
  );
