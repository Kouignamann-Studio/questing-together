...
import type { ActionId, CombatAction, OptionId, Scene, SceneId } from '@/types/story';
import { applyTagSet } from '@/utils/applyTagSet';
import { getDefaultNextSceneId } from '@/utils/getDefaultNextSceneId';
import { resolveNextSceneId } from '@/utils/resolveNextSceneId';
import { SCENE_BY_ID, STORY_CONFIG } from '@/utils/storyConfig';
...
function isSceneId(value: unknown): value is SceneId {
  return typeof value === 'string' && value in SCENE_BY_ID;
}
...
function computeCombatResolution({
  ...
  Object.entries(actionsByStep).forEach(([stepId, stepActions]) => {
    ...
    roundMap.set(
      round,
      [...stepActions].sort((a, b) => a.eventId - b.eventId),
    );
  });
...
function reduceStory(events: RoomEventRow[]): StoryReduction {
  ...
  const state: StoryReduction = {
    currentSceneId: STORY_CONFIG.startSceneId,
    sceneSequence: [STORY_CONFIG.startSceneId],
    actionsBySceneStep: {},
  ...
  sortedEvents.forEach((event) => {
    if (event.type === 'story_reset') {
      state.currentSceneId = STORY_CONFIG.startSceneId;
      state.sceneSequence = [STORY_CONFIG.startSceneId];
    ...
    if (event.type === 'scene_advance') {
      ...
      if (!state.resolvedOptionByScene[payload.sceneId]) {
        ...
        state.resolutionModeByScene[payload.sceneId] =
          scene?.mode === 'timed' || scene?.timed ? 'timed' : 'majority';
      }
...
export function useRoomStory({
  ...
  const combatConfig = STORY_CONFIG.combat ?? DEFAULT_COMBAT_CONFIG;
  ...
  const currentScene = SCENE_BY_ID[reduced.currentSceneId] ?? STORY_CONFIG.scenes[0];
  const storyInstanceKey = useMemo(() => {
    const latestResetEventId =
      [...events].reverse().find((event) => event.type === 'story_reset')?.id ?? 0;
  ...
  const combatSnapshot = isCombatScene
    ? (combatSnapshots.combatStatesByScene[currentScene.id] ?? null)
    : null;
  ...
  const firstIncompleteIndex = !isCombatScene
    ? stepStatuses.findIndex((item) => !item.completed)
    : -1;
  const currentStepIndex =
    !isCombatScene && stepStatuses.length > 0
  ...
  const currentStep =
    !isCombatScene && stepStatuses.length > 0
      ? (stepStatuses[currentStepIndex]?.step ?? currentScene.steps[0])
      : null;
  ...
  const sceneActionEvents = useMemo(() => {
    const allActions: SceneActionPayload[] = Object.values(actionsByStep)
      .flat()
      .filter(Boolean) as SceneActionPayload[];
  ...
  const disabledActionIds = useMemo(() => {
    ...
    currentStepActions.forEach((action) => {
      ...
      for (const id of outcome?.disableActionIds ?? []) disabled.add(id);
    });
    return disabled;
  ...
  const journalEntries = useMemo<JournalEntry[]>(() => {
    ...
    sequence.forEach((sceneId, index) => {
      ...
      const previousOption = previousSceneId
        ? (reduced.resolvedOptionByScene[previousSceneId] ?? null)
        : null;
      const introText =
        previousOption && scene.introByPreviousOption?.[previousOption]
          ? (scene.introByPreviousOption[previousOption] as string)
          : scene.intro;
      if (introText) {
        entries.push({
      ...
      const sceneActionsByStep = reduced.actionsBySceneStep[sceneId] ?? {};
      const sceneActionEvents: SceneActionPayload[] = Object.values(sceneActionsByStep)
        .flat()
        .filter(Boolean) as SceneActionPayload[];
      ...
      if (!(scene.mode === 'combat' || scene.combat)) {
        const actionMetaMap = new Map<
          ActionId,
          { text: string; buttonText?: string; stage?: string; narration?: string }
        >();
        ...
        sceneActionEvents.forEach((action, actionIndex) => {
          ...
          const stageLine = stageLineRaw
            ? `${stageLineRaw.charAt(0).toUpperCase()}${stageLineRaw.slice(1)}`
            : undefined;
          const hasSilentButtonLabel =
            Boolean(meta?.buttonText) && extractQuotedLines(actionText).length === 0;
          const spokenLines =
            ...
            action.actionId === NO_REACTION_ACTION_ID
              ...
              : spokenLines.length > 0
                ...
                : hasSilentButtonLabel
                  ...
                  : [actionText];
          const playerName =
            playerDisplayNameById[action.playerId] ?? playerNameById[action.playerId];
          ...
          npcDialogue.forEach((line, lineIndex) => {
            entries.push({
              ...
              narration:
                line.narration ??
                (lineIndex === npcDialogue.length - 1 ? outcome?.narration : undefined),
            });
          });
        });
      } else {
  ...
  const timedDurationSeconds =
    currentTimer?.durationSeconds ?? (isTimedScene ? (timedConfig?.durationSeconds ?? null) : null);
  ...
  const timedStatusText =
    isTimedScene && currentStepCompleted
  ...
  const unlockedOptionIds = useMemo(() => {
    ...
    currentScene.unlockRules.forEach((rule) => {
      const isUnlocked = rule.evidenceIds.every((evidenceId) =>
        confirmedEvidenceIds.has(evidenceId),
      );
    ...
    sceneActionEvents.forEach((action) => {
      const outcome =
        currentScene.steps
          .map((step) => step.outcomes[action.actionId])
          .find((value) => Boolean(value)) ?? null;
      for (const optionId of outcome?.unlockOptionIds ?? []) unlocked.add(optionId);
    });
  ...
  const visibleOptions = useMemo(() => {
    ...
    return currentScene.options.filter(
      (option) => option.defaultVisible || unlockedOptionIds.has(option.id),
    );
  }, [currentScene.options, isCombatScene, isTimedScene, unlockedOptionIds]);
  const hiddenOptionCount =
    isCombatScene || isTimedScene ? 0 : currentScene.options.length - visibleOptions.length;
  ...
  const localSelectedActionId =
    currentStepActions.find((action) => action.playerId === localPlayerId)?.actionId ?? null;
  ...
  const canVote =
    !isCombatScene && !isTimedScene && currentStepCompleted && !resolvedOption && !isStoryEnded;
